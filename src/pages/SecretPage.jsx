...
import { secretBattleBgm } from '../assets/bgm';
...
export default function SecretPage() {
  ...
  useEffect(() => {
    ...
    secret_pokemon.forEach((p, i) => {
      if (!pokemonId) return;
      const isCaught = pokedex.find((entry) => entry.id === pokemonId)?.catch === true;
      if (isCaught) return;
    ...
    const handleKeyDown = (e) => {
      ...
      if (e.code === 'KeyZ') {
        ...
        if (secretPokemonMapRef.current.has(checkKey)) {
          const pokedex = JSON.parse(sessionStorage.getItem('pokedex') || '[]');
          const cryUrl = pokedex.find((p) => p.id === pokemonId)?.cryUrl;
          if (cryUrl) {
          ...
          transitionRef.current.start(() => {
            ...
            console.log(pokemonId);
            navigate('/battle', { state: { pokemonId } });
          });
          return;
        }
      }
